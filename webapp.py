...
</html>"""PARCELS_HTML = “””<!DOCTYPE html>
<html lang="ru">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
...
function renderParcel(p) {
  ...
  return `<div class="parcel-card"><div class="parcel-header"><span class="track">🏷 ${p.track\_number}</span><span class="status ${cls}">${icon} ${p.status}</span></div><div class="parcel-info">${p.description ? `<div>📝 ${p.description}</div>` : ''}<div class="parcel-row">${p.weight ? `<span>⚖️ ${p.weight} кг</span>` : '<span></span>'}<span>📅 ${(p.created_at||'').substring(0,10)}</span></div></div></div>`;
}
...
function renderUsers(users) {
  ...
  el.innerHTML = users.map(u => `<div class="card"><div class="user-name">${u.full\_name}</div><div class="user-meta"><div>🔑 Код: <span class="code">${u.client_code}</span></div><div>📱 ${u.phone}</div><div>📅 ${(u.created_at||'').substring(0,10)}</div><div>🌐 ${u.language==='kg'?'Кыргызча':'Русский'}</div></div></div>`).join('');
}
